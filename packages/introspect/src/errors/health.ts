...
/**
 * Default hint texts used by the health interpreter.
 *
 * @remarks
 * These are generic, action-oriented fallbacks. Domain-specific code
 * (for example, helpers that switch on `error.code`) is free to replace
 * them with more precise guidance for a given error.
 */
...
/**
 * Default hint per health status and recoverability.
 *
 * @remarks
 * The aim is to answer “what should the operator do next?” in one line.
 * Callers may ignore or override this when they have richer context
 ...
 * current trust boundary (for example, to logs, UIs, or remote callers).
 *
 * @remarks
