...
> *The spec is the "truth of the device"; everything else is derived.*
...
In Seqlok, a **spec** is a *pure description* of:
...
Behavior and UI concerns live in *other* layers.
...
* `param` and `meter` are *builder namespaces*.
...
If you *must* support variable sizes:
...
* The *structure* (field names, types, shapes)
...
Some patterns are *possible* in TypeScript, but break Seqlok's design assumptions.
...
While you *can* derive UI from the spec, it's not meant to be a complete design system.
...
Everything else — plan, backing, handoff, bindings, concurrency — builds on top of this single, simple, *schema-first*
definition.
