...
```bash
pnpm -F @exclave/boundary run bench:report
```
...
Re-run `pnpm -F @exclave/boundary run bench:report` whenever you change benches or touch the hot paths.
...
```bash
pnpm -F @exclave/boundary exec vitest bench bench/observer-reads.bench.ts
```
